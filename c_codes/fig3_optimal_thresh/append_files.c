...
if(CONTINUATION == 1){				snprintf(stat_template, 256, "Context_STATISTICS_Dim%.3d_Pop%.2d_Gen%.2d_B%.2f_C%.2f_PM%.4f_SM%.4f_ICthresh%.5f_ICscaler%.5f_Tsigma%.5f_Ssigma%.5f_iteration%d_testscalerIC%lf", PHENOTYPE_DIMENSION, POPULATION, endtime, BENEFIT, COST, PHENOTYPE_MUTATION, STRATEGY_MUTATION, INITIAL_CONTEXTFREE_THRESH, INITIAL_CONTEXT_SCALER, tSIGMA, sSIGMA, current_iteration, TEST_SCALER_IC);}
			...
			*stat_output = fopen(stat_template, "w");
			...
			{
				fprintf(*stat_output, ", num_GENEROUS, num_STRICTs, num_CF, num_C, avg_contextfree_thresh, std_contextfree_thresh, avg_context_scaler, std_scaler, DONATIONS, avg_acting_thresholds_context, mean_payoffs_CF, mean_payoffs_C, all_thresholds, all_threshold_coefficients");
		...
		for(int island = 0; island < ISLANDS; island++)
		{
			...
			if(fitnesses_on == true)
	        	{       
        			...
        			fprintf(*fitness_output, "TIME, INDIVIDUAL, DISTANCE_TO_MEAN, CONTEXTFREE_THRESH, CONTEXT_SCALER, SCORE, DONATIONS, acting_threshold, CF_or_C, optimal_threshold, average_distance_observed, numerical_thresh, numerical_score");
	...
	for(int island = 0; island < ISLANDS; island++)
        {
                ...
                for (int i = 0 ; i < POPULATION; i++)
                {
                        ...
                        average_distances[i][island] = average_distance;
			...
			for(int island = 0; island < ISLANDS; island++)
			{
				...
				for (int i = 0; i < PHENOTYPE_DIMENSION; i++)
				{
					...
					double distance = 0;
                	                ...
                	                fprintf(*fitness_output, "%d, %d, %.5f, %.5f, %.15f, %.5f, %d, %.15f, %d, %.15f, %.15f, %.15f, %.15f", time, i, distance, thresholds[i][island], scalers[i][island], results[i][island], donation_events[i][island], acting_thresholds[i][island], cf_or_c[i][island], hamilton_distances[i][island], average_distances[i][island], numerical_thresh[i][island], numerical_highscore[i][island]);
					fprintf(*fitness_output, "\n");
		...
		if(stat_on == true && time%STEP_SIZE == 0)
			...
			for(int island = 0; island < ISLANDS; island++)
			{
				...
				for(int i = 0; i < POPULATION; i++)
				{
					all_thresholds += thresholds[i][island];
					all_threshold_coefficients += scalers[i][island];
					/*if(rand()%50==0)
					{
				...
				fprintf(*stat_output, ", %d, %d, %d, %d, %f, %f, %f, %f, %d, %f, %f, %f, %f, %f", num_generous, num_stricts, num_CF, num_C, CF_mean, CF_std, C_mean, C_std, total_donations[island], acting_threshold_avg, CF_payoff_mean, C_payoff_mean, all_thresholds, all_threshold_coefficients);
