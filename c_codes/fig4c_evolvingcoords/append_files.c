...
if(CONTINUATION == 1){				snprintf(stat_template, 256, "Context_STATISTICS_Dim%.3d_Pop%.2d_Gen%.2d_B%.2f_C%.2f_PM%.4f_SM%.4f_ICthresh%.5f_ICscaler%.5f_Tsigma%.5f_Ssigma%.5f_iteration%d_testscalerIC%lf", PHENOTYPE_DIMENSION, POPULATION, endtime, BENEFIT, COST, PHENOTYPE_MUTATION, STRATEGY_MUTATION, INITIAL_CONTEXTFREE_THRESH, INITIAL_CONTEXT_SCALER, tSIGMA, sSIGMA, current_iteration, TEST_SCALER_IC);}
	...
	}else if(time <= endtime)
	{
		...
		if(fitnesses_on == true && time%FITNESS_STEP_SIZE == 0 /*&& time > 5000 && time < 6000*/)
               	{
		...
		if(stat_on == true && time%STEP_SIZE == 0)
			...
			for(int island = 0; island < ISLANDS; island++)
			{
				...
				/* simplify, just make the mean the averages of those rows/columns, not selecting by active/not */

				double n_justC[POPULATION];
				double n_justCF[POPULATION];

				for(int i = 0; i < POPULATION; i++)
				{  
					n_justC[i] = scalers[i][island];
					n_justCF[i] = thresholds[i][island];
				} 					

				CF_mean = gsl_stats_mean(n_justCF, 1, POPULATION);
                                CF_std = sqrt(gsl_stats_variance(n_justCF, 1, POPULATION));

				C_mean = gsl_stats_mean(n_justC, 1, POPULATION);
                                C_std = sqrt(gsl_stats_variance(n_justC, 1, POPULATION));
