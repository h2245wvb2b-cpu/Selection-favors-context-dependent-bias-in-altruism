	...
	printf("time, rand_individual, average_distance, cf_or_c, threshold, scalers, num_duplicates, T0, P0, part_1, T1, P1, part_2, T2, P2, part_3, T3, P3, part_4, T4, P4, part_5, T5, P5, part_6, T6, P6, part_7, T7, P7, part_8, T8, P8, part_9, T9, P9, part_10, T10, P10\n");
		...
		if(time%FITNESS_STEP_SIZE == 0 && EMPIRICAL_THRESH == 1 && (time > 10000 && time < 10500))
		{
			...
			for(int i0 = 0; i0 < 1000; i0++)
			{
				...
				int rand_type = cf_or_c[rand_individual];
				...
				for(int i = 0; i < POPULATION; i++)
				{
					...
					if(cf_or_c[i] == rand_type && rand_type == 1)
					{
						if(thresholds[rand_individual] == thresholds[i])
						{
							num_A++;
					...
					if(cf_or_c[i] == rand_type && rand_type == -1)
					{
						...
						if(scalers[rand_individual] == scalers[i])
						{
							num_A++;
				...
				for(int i = 0; i < SURVEY; i++)
				{
					...
					{
						distance_to_survey += (phenotypes[rand_individual][j] - phenotypes[ store[rand_individual][i]][j])*(phenotypes[rand_individual][j] - phenotypes[store[rand_individual][i]][j]);
					}
					...
					if(distance_to_survey < acting_thresholds[rand_individual])
					{
						results_copy[ store[rand_individual][i] ] -= BENEFIT;
					} else
					{
			...
			printf("%d, %d, %f, %d, %f, %f, %d", time, rand_individual, ave_pairwise_dist, cf_or_c[rand_individual], thresholds[rand_individual], scalers[rand_individual], num_A);
			...
			for(int i = 0; i <= SURVEY; i++)
			{
				...
				for(int j = 0; j < num_A; j++)
				{
					if(i != 0)
					{						
						if(As[j] == store[rand_individual][ascending_distance[i-1]])
		                                {
				...
				if(i < SURVEY)
				{
					...
					printf(", %f, %f, %d", distances_to_partners[ ascending_distance[i] ], delta_p, store[rand_individual][i]);
				}
